...
| 平台 | 最低版本 | 推荐配置 | 说明 |
|------|----------|----------|------|
| macOS | 26.0 | M 系列芯片必需；16 GB 内存（可用 ≥ 6 GB）；可用磁盘 ≥ 24 GB | 原生 macOS 应用（SwiftUI） |
| Windows | Windows 10 22H2 / 11（建议 24H2 及以上） | 最低 20 GB 可用显存；建议固态硬盘；可用磁盘 ≥ 26 GB | Core API 仅支持；无原生 UI |
| iOS | iOS >= 26.0 | 需连接运行隙间最新版本的计算机，远程使用全部功能 | 无原生客户端，仅伴侣端 |
| Android | Android >= 12.0 | 需连接运行隙间最新版本的计算机，远程使用全部功能 | 无原生客户端，仅伴侣端 |
| Linux | — | — | 暂不支持（Core API 理论可跑，但无 UI 前端） |
