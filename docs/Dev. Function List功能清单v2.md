...
## v2.1 关键决策（2026-06-25 更新）（部分内容将会在后续修改，主要会修改的是对于模型的选择，所以关于模型的选择的部分请自行忽略，谢谢）
...
- **边界场景**
  - 高频事件风暴：必须节流（默认 60s 内最多 1 个事件；已实装为 `DEFAULT_GLOBAL_COOLDOWN_SECONDS=60`，高于优先级的 loser 会被标 `lost_priority_race`）
...
- **验收标准**
  - AC-1：OOC 触发率 < 1%（✅ 已验证：`docs/eval/safety_eval.jsonl` 105 条正常 + 35 条 OOC 对照，`python -m core.scripts.eval_safety` 输出正常集触发率 0.00% < 1%，OOC 检出率 100%）
  - AC-2：危险场景例外必须显式记录原因
  - AC-3：所有拦截事件必须可查询
- **边界场景**
...
| **v2.8** | 2026-07-20 | **A5.2 MCP 防护实装**（从零起，无前置 stub/路由）：新增 `stubs/mcp_rules.py`（8 种 `action_kind`：`file_delete` / `file_write` / `file_read` / `shell` / `network` / `app_launch` / `settings_modify` / `system_cmd` + 2 种 `mode`：`blacklist` / `whitelist` + `severity ∈ [1, 5]` + `is_active` A/B 开关 + broken-regex 静默跳过 + 热路径 `match_action_rules` 按 severity desc 排序 + unknown kind 拒绝匹配）、`stubs/mcp.py`（三大子系统：**Gate** `check()` 决策树按"overload 短路放行 / lockout 短路 deny / 冻结 deny / blacklist hit deny / whitelist hit allow / no match 走 per-world policy (default=deny/allow)" 7 个分支 + self-crash fallback `denied_crashed`；**Safety-stop state machine** `safety_stop` / `confirm_safety_stop` / `cancel_safety_stop` 6 状态（`frozen` / `awaiting_confirm` / `sanitizing` / `restored` / `cancelled` / `lockout`），3 次 60s 内连续触发 → `lockout` 状态，409 拒绝后续 safety_stop 直到 `clear_lockout` 冷重启重置（含 freeze history）；**Snapshots** `dump_snapshot` 覆盖 `state.{worlds,characters,memory,sessions}` 4 个受保护 bucket（AC-4）+ 深拷贝隔离 + `sanitize_snapshot` 复用 A5.1 `forbidden_word` 规则做字段级 scrub（输出 `[sanitized]`） + `restore_snapshot` 自动 sanitize 兜底）。audit log 每次 check 一条，3 个 module-level `_seq` 计数器（audit / freeze / snapshot）保证同秒插入的稳定排序。路由全挂 `/v1/xijian/mcp/*`（check + rules CRUD + audit list/count + per-world policy get/put/delete + safety_stop init/list/get/confirm/cancel + snapshots list/get/dump/sanitize/restore + dev crash 演练端点，XIJIAN_DEV=1 gate）。`utils/ids.py` 加 `gen_mcp_rule_id` / `gen_mcp_audit_id` / `gen_mcp_freeze_id` / `gen_mcp_snapshot_id`（前缀 `mcpr_` / `mcpa_` / `mcpf_` / `mcpsnap_`）。**AC-2（"安全终止响应延迟 < 200ms"）和全局快捷键（⌃⌥⌘Q / Win+Alt+Shift+Q）监听没接**——spec 标 `[TODO]`，server 端只负责策略权威性 + 状态机；快捷键 + UI 确认弹窗 + MCP 进程 SIGFREEZE 属于桌宠客户端（Pywebview）范畴。补 262 个测试覆盖（105 mcp_rules + 157 mcp，含纯函数 / CRUD / 决策树全 7 分支 / 3-in-60s lockout / lockout 持久化 / clear_lockout 重置 / safety_stop 全状态 / snapshot dump+deep-copy+sanitize+restore 自动兜底 / 跨世界隔离 / per-world policy / HTTP / 鉴权 / dev crash）。core 全套 **1456** 测试通过、0 回归。实现细节、未做项与原因、跨章节联动见 `docs/notes.md` 2026-07-20 条目。 |
